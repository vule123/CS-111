...
#define TASKBUFSIZ 73728	// Increase the buffer size to handle more peers, 72KB
#define FILENAMESIZ	256	// Size of task_t::filename
#define MAXDOWNLOADSIZ 134217728	// Maximum file size that can be downloaded, 128MB
...
* Try 'rm %s.~*~' to remove them.\n", t->filename, t->filename);
	...
	// TASK 2B START
	// Limit the file size that can be downloaded
	if(t->total_written > MAXDOWNLOADSIZ){
		error("File exceeds the allowed download size, cannot download");
		goto try_again;
	}
	// TASK 2B END
...
static void task_upload(task_t *t)
{
	...
	// TASK 2B START
	// Make sure the peer serves files within the current directory and some validation
	// checking of the file and the file's path
	char file_path_buf[PATH_MAX + 1];
	char cur_path_buf[PATH_MAX + 1];
	char* file_path = realpath(t->filename, file_path_buf);	// get real path of the file
	char* cur_path = getcwd(cur_path_buf, PATH_MAX + 1);	// get the current directory

	// Check that the current directory and the file path are valid 
	if(cur_path == NULL){
		errno = ENOENT;
	...
	// Check that the filename passed by the peer is not too long
	if(strlen(t->buf) > FILENAMESIZ + 12){
		error("GET request is too long");
		goto exit;
	}
	...
	// Check that the file is in the current directory before uploading
	// If encountering a null character, break out of the while loop
	int notCurDir = 0;
	int i = 0;
	while(i < FILENAMESIZ){
		if(t->filename[i] == '\0')
	...
	if(notCurDir){
		errno = ENOENT;
		error("The file is not in the current directory, cannot upload");
		goto exit;
	}
	// TASK 2B END
...
"         -dDIR        Upload and download files from directory DIR.\n"
	...
	for(; argc > 1; argc--, argv++){
		if((t = start_download(tracker_task, argv[1]))){
			...
			if(pid >= 0){	// fork succeeded
				if(pid == 0){	// child process downloads in parallel with other child processes, then exit
					task_download(t, tracker_task);
					_exit(0);
				}
				else{	// parent process
				...
				error("Failed to fork, cannot download");
			}
		}
	}
	...
	while((t = task_listen(listen_task))){
		pid = fork();
		if(pid >= 0){	// fork succeeded
			if(pid == 0){	// child process uploads in parallel with other child processes, then exit
				task_upload(t);
				_exit(0);
			}
			else{	// parent process
			...
			error("Failed to fork, cannot upload");
		}
	}
	// TASK 1 END
