...
static pid_t make_new_thread(process_t *parent, int start_function);
...
interrupt(registers_t *reg)
{
	...
	case INT_SYS_EXIT:
		...
		if(current->wait_queue > 0)
	        {
		    proc_array[current->wait_queue].p_state = P_RUNNABLE;
		    proc_array[current->wait_queue].p_registers.reg_eax = current->p_exit_status; 
		}
	...
	case INT_SYS_WAIT: {
		...
		{
		        proc_array[p].wait_queue = current->p_pid;
	...
	case INT_SYS_NEW_THREAD:
	        current->p_registers.reg_eax = make_new_thread(current, current->p_registers.reg_eax);
	        run(current);
	...
	case INT_SYS_KILL:
	{
	  ...
	  if(p <= 0 || p >= NPROCS || p == current->p_pid || proc_array[p].p_state == P_EMPTY)
	      current->p_registers.reg_eax = -1;
	  else if(target->p_state != P_ZOMBIE)
	  {
	      target->p_state = P_ZOMBIE;
	      target->p_exit_status = -1;
	      if(target->waiting_proc != NULL)
	      {
		  target->waiting_proc->p_state = P_RUNNABLE;
		  target->waiting_proc->p_registers.reg_eax = target->p_exit_status;
...
do_fork(process_t *parent)
{
	...
	// Finally, return the child's process ID to the parent.
        pid_t count = 1;
	for(; count < NPROCS; count++)
	{
	    if(proc_array[count].p_state == P_EMPTY)
	    {
	        process_t *child = &proc_array[count];
...
copy_stack(process_t *dest, process_t *src)
{
	...
	memcpy((void *)dest_stack_top, (const void *)src_stack_top, src_stack_size);
...
{
  pid_t pid = 1;
  process_t *new_thread;
  do
  {
    new_thread = &proc_array[pid];
    if(new_thread->p_state == P_EMPTY)
      {
	new_thread->p_registers = parent->p_registers;
